...
| Item | Answer |
|---|---|
| OJ problem number/title | OJ3071 - จำนวนในช่วง [A,B] ที่หารด้วย d เหลือเศษ r |
| OJ submission ID, if submitted | 573797 |
| OJ status | Pass |
...
```text
ฉันถามAI ว่ามีวิธีไหนที่สามารถแก้การใช้loop แล้วใช้เวลาในการรันนานเกินไปมั้ย
```
...
```text
AI ช่วยบอกว่าไม่ต้องเก็บข้อมูลไว้ในลิสก็ได้ให้ใช้ loopรันหาคำตอบจะใช้เวลาน้อยลง
```
...
```text
ฉันลองแก้โดยใช้for loop เช็คทีละตัวโดยลบ1ไปทีละตัวเลย
```
...
```text
ได้เรียนรู้ว่าเราไม่จำเป็นที่จะต้องเก็บข้อมูลไว้ในลิสเสมอเพราะอาจใช่เวลานานเมื่อรัน
```
